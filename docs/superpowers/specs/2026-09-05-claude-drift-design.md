...
- `--settings`로 주입한 설정은 사용자 훅과 플러그인을 끄지 못한다. 병합될 뿐이다 (11장 실측). 도구 실행 차단은 첫 tool_use 수신 시 프로세스를 죽이는 것이 1차 수단이고, 같은 설정의 PreToolUse 거부 훅은 그것이 실패했을 때의 2차 안전장치다. `--disallowedTools`는 쓰지 않는다. 모델의 선택지를 바꾸기 때문.
...
- **해결됨 (2026-09-05 실측, Claude Code 2.1.261, `tests/test_replay_live.py`).** `--settings`는 **덮어쓰기가 아니라 병합**이다. 우리가 넣은 설정 파일에는 `PreToolUse` 거부 훅 하나뿐인데, 재생 프로세스의 stream-json 첫 8줄이 사용자 환경의 `SessionStart:fork` 훅 4개가 실행된 기록이었다 (`{"type":"system","subtype":"hook_response","hook_name":"SessionStart:fork","output":"{\"continue\":true,\"suppressOutput\":true}","exit_code":0}`). `init` 이벤트의 `tools` 개수도 461개로, 플러그인과 MCP 서버가 전부 그대로 로드된다. 즉 `--settings`로 사용자 환경을 격리할 수 없고, 거부 훅을 **추가**할 수 있을 뿐이다. 재생 결과는 이 로컬 플러그인 구성에 의존한다는 뜻이므로, 결과를 볼 때 항상 같은 기계·같은 플러그인 구성이라는 전제를 명시한다. 한편 거부 훅 자체는 **이번 실측에서 발동하지 않았다**: 우리는 첫 `tool_use` 블록이 스트림에 나타난 순간 프로세스를 죽이기 때문에 도구 실행 단계(`PreToolUse`)까지 도달하지 않는다. 두 스트림 모두 `PreToolUse` 훅 이벤트도, 차단 메시지가 담긴 `user` tool_result 이벤트도 없다. 거부 훅은 조기 종료가 실패했을 때만 쓰이는 2차 안전장치로 남는다. (`UserPromptSubmit` 훅은 사용자 설정과 플러그인 양쪽에 있지만 두 스트림 어디에도 이벤트가 없었다. `-p` 모드에서 실행되지 않는 것인지 스트림에 실리지 않는 것인지는 구분하지 못했고, 도구 차단과는 무관하므로 추적하지 않는다.)

- **해결됨 (2026-09-05 실측).** 같은 세션의 두 컷(같은 파일의 32줄 프리픽스, 218줄 프리픽스)을 순서대로 재생한 결과, 캐시는 재사용되지만 이득은 크지 않다. 첫 컷은 `cache_creation_input_tokens=35,052` / `cache_read_input_tokens=18,759`, 둘째 컷은 `cache_creation_input_tokens=54,139` / `cache_read_input_tokens=24,523`이었다. 첫 컷이 이미 18.7k를 읽은 것은 다른 로컬 Claude Code 사용이 만들어 둔 시스템 프롬프트 캐시 덕분이고, 둘째 컷이 첫 컷 덕에 추가로 읽은 양은 약 5.8k 토큰(둘째 컷 입력의 약 10%)에 그친다. 뒤 컷일수록 프리픽스가 길어져 대부분이 새 캐시 생성이 되기 때문이다. 결론: 같은 세션 오름차순 정렬은 **유지한다**. 공짜이고 손해가 없으며 소폭이라도 읽기가 늘지만, 비용 절감 근거로 삼지는 않는다. 큰 절감을 기대하려면 줄 간격이 가까운 컷들을 골라야 한다. (참고로 두 재생은 각각 10.9초, 31.3초가 걸렸다.)
- **미확인, 그대로 둠.** 세션 로그 형식은 Claude Code 버전에 따라 바뀔 수 있다. `version` 필드를 읽어 지원 범위를 명시하고, 파싱 실패는 세션 단위로 건너뛴다.
